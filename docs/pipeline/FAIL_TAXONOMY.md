...
- Jeden chain = konzistentní lineage (`runId`, `sourceRunId`, `reviewRunId`, `stagingRunId`, `gateRunId` podle kroku)
...
- Curation report (`tmp/curation/<runId>.curation.json`) indikuje `summary.acceptedCandidates > 0`, zatímco review decision set je prázdný.

Stop rule:
- Může pokračovat pouze jako explicitní no-op, pokud je prázdná queue očekávaná a auditně potvrzená.
- Pokud `summary.acceptedCandidates > 0`, pipeline musí zastavit (flow mismatch).
...
Detection:
- `tmp/publish-reports/<runId>.publish.json` má `summary.published > 0` a `summary.failed = 0`.
- Následující krok selže pro stejný lineage (např. sanitize hlásí missing snapshot / missing publish report / DB auth error).
...
Typ: operator-error
Důvod:
Lineage reference mezi artifacty nesedí (nejde jen o prosté porovnání jednoho `runId`).

Detection:
- `staging.sourceRunId` nesedí s `review.sourceRunId` nebo `curation.sourceRunId`.
- `publish.gateRunId` / `publish.stagingRunId` / `publish.reviewRunId` nesedí na gate a staging lineage reference.

Stop rule:
- Pipeline musí zastavit; pokračování je povoleno až po opravě lineage referencí mezi artifacty.

Actionable next step:
Sjednotit lineage reference (`sourceRunId`, `reviewRunId`, `stagingRunId`, `gateRunId`) napříč artifacty a zopakovat chain konzistentně.
...
2. Sedí lineage reference napříč kroky?
...
Stage:
- approved 0 + acceptedCandidates > 0 → bug

Gate:
- failure wrapper (`status=failed`) nebo invalid gate schema → vrátit se zpět
